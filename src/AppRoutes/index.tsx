...
// ==============================|| ROUTING RENDER ||============================== //
let routes = [LoginRoutes, MainRoutes];
console.log(process.env.REACT_APP_STORAGE_KEY, 'process.env.STORAGE_KEY');

const result = JSON.parse(localStorage.getItem(`${process.env.REACT_APP_STORAGE_KEY}/auth`) || '{}');
if (result && result?._id) {
    routes = [MainRoutes, LoginRoutes];
}

let AppRouter = createHashRouter(routes, { basename: process.env.REACT_APP_BASE_NAME });
